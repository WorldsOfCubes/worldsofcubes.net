...
switch ($do) {
	case 'write':
		...
		if (isset($_GET['name']))
			$name = TextBase::HTMLDestruct($db->safe($_GET['name'])); elseif (isset($_POST['name']))
			$name = TextBase::HTMLDestruct($db->safe($_POST['name']));
		else $name = false;
		if (isset($_POST['topic']))
			$topic = TextBase::HTMLDestruct($db->safe($_POST['topic'])); else $topic = false;
		if (isset($_POST['message']))
			$message = TextBase::HTMLDestruct($_POST['message']); else $message = false;
		$info = '';
		if (isset($_POST['submit'])) {
			loadTool("ajax.php");
			if (!CaptchaCheck(0, false))
				$info .= lng('CAPTCHA_FAIL').". ";
			if (!$topic or !$message or !$name)
				$info .= lng('INCOMPLETE_FORM').". ";
			if ((mb_strlen($name, "utf-8") < 3) or (mb_strlen($name, "utf-8") > 32))
				$info .= lng('INCORRECT_LEN_RECIVER').". ";
			if ((mb_strlen($message, "utf-8") > 1024) or (mb_strlen($message, "utf-8") < 5))
				$info .= lng('INCORRECT_LEN_MESSAGE').". ";
			if ((mb_strlen($topic, "utf-8") < 4) or (mb_strlen($topic, "utf-8") > 128))
				$info .= lng('INCORRECT_LEN_TOPIC').". ";
			$pl = new User($name, $bd_users['login']);
			if (!$pl->id())
				$info .= lng('INCORRECT_UNAME').". ";
			if (!(strlen($info) > 0))
				PManager::SendPM($user->name(), $pl, $topic, $message);
			if ((strlen($info) > 0))
				$info = View::Alert($info); else $info = View::Alert(lng('SENT_SUCCESS'), 'success');
		}
		ob_start();
	...
	case 'view':
		...
		if (isset($_GET['id']))
			$id = $_GET['id']; elseif (isset($_POST['id']))
			$id = $_POST['id'];
		else accss_deny();
		...
		if ($user->name() == $pm['reciver'])
			$pl = new User($pm['sender'], $bd_users['login']); else
