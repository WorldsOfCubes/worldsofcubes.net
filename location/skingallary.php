...
$user_admin = (!empty($user) and $user->lvl() >= 15) ? true : false;
...
if ((!empty($_POST['do']) or !empty($_GET['do'])) and !empty($user)) {
	...
	switch ($do) {
		...
		case 'add':	
			...
			if ($result < 0) $error = 'Скин уже есть в базе. <a href="index.php?mode=skingallary&cid='.($result * -1).'">Перейти</a>';
			else
...
$work_url = 'index.php?mode=skingallary'; $url_params = '';
