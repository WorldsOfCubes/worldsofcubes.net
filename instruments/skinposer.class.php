...
private $user_id;
	...
	global $bd_names, $site_ways, $config, $db;	
		...
		$result = $db->execute("SELECT `name`, `fname`, `fsize`, `dislikes`, `likes`, `ratio`, `gender`, `downloads`, `comments`, `user_id`, `comment_last`  FROM `{$this->db}` WHERE `id`='". $this->id ."'"); 
	...
	public function OnComment() {
		...
		$db->execute("UPDATE `{$this->db}` SET `comments` = '".$this->comments."', `comment_last` = '".$this->comment_last."' WHERE `id`='". $this->id ."'");			
	}
	...
	public function OnDeleteComment() {
		...
		$db->execute("UPDATE `{$this->db}` SET `comments` = '".$this->comments."' WHERE `id`='". $this->id ."'");			
	}
	...
	if ($method == 'post') {
		...
		if ($del_blist) $db->execute("DELETE FROM {$this->db_bad_skins} WHERE hash='".$hash."'");
		
		$result = $db->execute("SELECT `id`, 'good_skin' AS `type` FROM `{$this->db}` WHERE hash='".$hash."' UNION SELECT `id`, 'bad_skin' AS `type` FROM {$this->db_bad_skins} WHERE `hash`='".$hash."'");
		...
		$db->execute("INSERT INTO `{$this->db}` (hash, fsize, ratio, gender, user_id) VALUES ('".$hash."','".$size_mb."','".$new_file_ratio."', '".$gender."', '".$user_id."')");
		
		$this->id = $db->insert_id();
		$new_name = 'sp_nc'.$this->id.'.png';
		$new_way = $this->base_dir.$new_name;
		
		$db->execute("UPDATE `{$this->db}` SET `fname` = '".$new_name."' WHERE `id`='".$this->id."'");
		
		if (file_exists($new_way)) unlink($new_way);
		
		if (rename( $way, $new_way )) chmod($new_way , 0777);
		else { unlink($way); $db->execute("DELETE FROM `{$this->db}` WHERE `id`='".$this->id."'"); return 6; }	
		
		$preview = skinGenerator2D::savePreview($this->base_dir.'preview/'.$new_name, $new_way, false, false, 160);
		if (!$preview) { unlink($new_way); $db->execute("DELETE FROM `{$this->db}` WHERE `id`='".$this->id."'"); return 7; }	
		else imagedestroy($preview);
		
		$db->execute("LOCK TABLES `{$this->db_ratio}` WRITE;");
		$db->execute("INSERT INTO `{$this->db_ratio}` (ratio) VALUES ('".((int)$new_file_ratio)."') ON DUPLICATE KEY UPDATE `num`= num + 1;");
	...
	public function Rebuild() {
		...
		$db->execute("LOCK TABLES `{$this->db_ratio}` WRITE;");
		$db->execute("INSERT INTO `{$this->db_ratio}` (ratio) VALUES ('".((int)$skin_ratio)."') ON DUPLICATE KEY UPDATE `num`= num + 1;");
		$db->execute("UNLOCK TABLES;");

		if ($this->ratio != $skin_ratio) {
		
			$db->execute("UPDATE `{$this->db}` SET `ratio` = '".$skin_ratio."' WHERE `id`='".$this->id."'");
	...
	public function SetGender($new_gender) {
		...
		$db->execute("UPDATE `{$this->db}` SET `gender` = '$new_gender' WHERE `id`='".$this->id."'");
	...
	global $bd_names, $db;
		...
		$result = $db->execute("SELECT `hash` FROM `{$this->db}` WHERE `id`='".$this->id."'");
		
		if ($db->num_rows( $result )) {
			
		$line = $db->fetch_array( $result );
		$db->execute("INSERT INTO {$this->db_bad_skins} (hash) VALUES ('".$line['hash']."')");
		}
		
		if (file_exists($skin_way)) unlink($skin_way);
		if (file_exists($preview_way)) unlink($preview_way);
		
		$db->execute("DELETE FROM `{$this->db_likes}` WHERE `item_id`='".$this->id."' AND `item_type`='".ItemType::Skin."'");
		
		$db->execute("LOCK TABLES `{$this->db_ratio}` WRITE;");
		$db->execute("UPDATE `{$this->db_ratio}` SET `num` = num - 1 WHERE `ratio`='".$this->ratio."'");
		$db->execute("UNLOCK TABLES;");
...
private $answer;
    ...
    public function SkinManager($style_sd = false, $base_url = 'index.php?mode=skingallary', $url_params = false) {
	...
	$db->execute("LOCK TABLES `{$this->db_ratio}` WRITE;");
	$db->execute("DELETE FROM `{$this->db_ratio}` WHERE `ratio` != '0'");	
	$db->execute("UNLOCK TABLES;");
	
	@ini_set("max_execution_time", 0);
	
		$result = $db->execute("SELECT `id` FROM `".$this->db."`");
	...
	global $db;
		global $db;
		if ($db->execute("SELECT `$by_column` FROM `$table` LIMIT 0, 1")) return true;
	...
	global $config, $bd_names, $menu;
		...
		$tool_sp_btn = array (		
					'name'			=> 'Образы',
					'url' 			=> ($config['rewrite'])? 'go/skinposer' : '?mode=skingallary',
					'parent_id'		=> -1,
					'lvl'			=> 1,
					'permission'	=> -1,
					'config'		=> 'sp_online');
	...
	if (!empty($user) and $user->lvl() >= 15) $admin = true;
				
	$result = $db->execute("SELECT `ratio`,`num` FROM `".$this->db_ratio."` ORDER BY `ratio` LIMIT 0, 90");
	...
	global $db;
	$skins_count = $db->fetch-array($db->execute("SELECT COUNT(*) FROM {$this->db}"), 0);
	$skins_count = $skins_count[0];
	if ( !$skins_count ) return $this->ShowById(0); //
	
	$result = $db->execute("SELECT `id` FROM {$this->db} LIMIT ".rand(0, $skins_count-1).", 1");
	...
	global $user, $db;		
		...
		if ($mode != 'likes') {
			...
			$skins_count = $db->result($db->execute("SELECT COUNT(*) FROM `".$this->db."` ".$base_sql), 0);
			$skins_count = $skins_count[0];
			...
			$skins_count = $db->result($db->execute("SELECT COUNT(*) FROM `".$this->db_likes."` WHERE `user_id`= '".$user->id()."' AND `item_type` = '".$this->type."' AND `var`='1'"), 0);
			$skins_count = $skins_count[0];
		}
		...
		if (!$skins_count) $html_skin_list = $this->ShowPage('skin_empty.html');
			...
			$result = $db->execute("SELECT `id` FROM `".$this->db."` ".$base_sql." ORDER BY `$order_by` $sort LIMIT ".($per_page*($list-1)).",$per_page");
			...
			if ( !$db->num_rows( $result )) $html_skin_list = $this->ShowPage('skin_empty.html');
