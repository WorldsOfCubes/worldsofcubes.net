...
class Comments_Item extends Item {
	...
	public function aCreate($message, User $fUser, $item_id, $item_type, $antibot = 'antibot') {
		...
		if ($rcode == 1701)
			aExit(1, lng('MESS_SHORT')); elseif ($rcode == 1702 or $rcode == 1703)
			aExit(2, lng('MESS_NOT_FOUND'));
		elseif ($rcode == 1) {
	...
	public function Show($for_user = false) {
		global $db;
		if (!$this->Exist())
			return $this->ShowPage('comment_not_found.html');
		...
		$verified    = ($this_user->verified())? ' <div class="label label-success stt" data-toggle="tooltip" title="Подтвержденный ник"><i class="glyphicon glyphicon-ok"></i></div>':'';
		...
		ob_start();
		if ($for_user)
			include $this->GetView('comments.html'); else           include $this->GetView('comments_unauth.html');
