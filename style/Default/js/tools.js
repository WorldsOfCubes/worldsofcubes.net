...
function SendByXmlHttp(script, post_data, onload) {
	...
	req.onreadystatechange = function () {
		...
		if (req.readyState != 4 || (req.status != 200 && req.status != 0) || (req.status == 0 && req.responseText.length == 0)) return false
...
if (typeof $ == 'function')
	...
	$(function () {
		...
		$('input[type=file]').each(function (i, elem) {
			...
			$(elem).replaceWith('<a class="file-input-wrapper btn btn-default">' + buttonWord + input + '</a>');
			...
			.promise().done(function () {
				...
				$('.file-input-wrapper').mousemove(function (cursor) {
					...
					input.css({
						left: moveInputX, top: moveInputY
					});
				});
