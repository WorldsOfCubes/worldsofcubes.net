...
function SubmitNewProject() {
	...
	GetById('update-status-icon').className = 'fa fa-refresh fa-spin'
...
//		GetById('project-add').reset()
		...
		if (response != null) {
			...
			if (response['code'] == 0) {
				...
				GetById('update-status-icon').className = 'glyphicon glyphicon-ok'
				$('new-project-link').attr('href', "go/project/edit/" + response['message'])
				GetById('project-add-id').innerHTML = response['message']
				GetById('project-add-key').innerHTML = response['security-key']
				BlockVisible('project-add-error', false)
				return
			}
			if (response['code'] == 100) {
				BlockVisible('project-add-error', false)
			...
			GetById('update-status-icon').className = 'glyphicon glyphicon-remove'
			if (response['code'] != 3)$('#collapseOne').collapse('toggle');
			GetById('project-add-error-text').innerHTML = nl2br(response['message'])
			BlockVisible('project-add-error', true)

		} else {

			$('#collapseOne').collapse('toggle');
			GetById('project-add-error-text').innerHTML = err404 + req.status
			BlockVisible('project-add-error', true)

		}
	}

	sendFormByIFrame('project-add', event)
	return false
}
...
function RemoveTrust(id) {
	...
	var event = function(response) {
		...
		BlockVisible('main-error', false  )
		$('#trusted-project-' + id).fadeOut(200, function (){

			$(this).hide()
			var keyBin = GetById('trusted-project-' + id)
	...
	SendByXmlHttp('action.php', 'method=trust_delete&pid=' + encodeURIComponent(id), event)
	return false
}

function RemoveAllTrust(id) {
	...
	var event = function(response) {
		...
		BlockVisible('main-error', false  )
		$('#trusted-projects').fadeOut()
		$('#trusted-projects').html('<tr><td colspan="2" align="center">Нет ничего :(</td></tr>')
		$('#trusted-projects').fadeIn()
	}

	SendByXmlHttp('action.php', 'method=trust_full_cleanup', event)
	return false
}