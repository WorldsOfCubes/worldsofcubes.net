...
function SetGenderSkin(id, current) {
	...
	SendByXmlHttp('index.php', 'mode=skingallary&do=gender&skin_id=' + encodeURIComponent(id) + '&new_gender=' + set, event)
	return false
}
...
function DeleteSkin(id) {
	...
	SendByXmlHttp('index.php', 'mode=skingallary&do=del&skin_id=' + encodeURIComponent(id), event)
	return false
}
...
function ApplySkin() {
	...
	SendByXmlHttp('index.php', 'mode=skingallary&do=get&skin_id=' + encodeURIComponent(skin) + '&antibot=' + code, event)
	return false
}
...
function uploadSkin() {
	...
	var event = function (response) {
		...
		if (response != null) {	
			...
			document.location.href = base_url + 'index.php?mode=skingallary&method=1&ratio=' + response['ratio'] + '&type=' + response['gender']
			return false			
			}
